...
[Subcommand]
[Command("editorconfig", Description = "Add default .editorconfig")]
public class Editorconfig
{
    ...
    public async Task Default(DefaultOption option)
    {
        ...
        var editorConfigFilePath = Path.Combine(projectRootDirectoryPath, ".editorconfig");

        if (systemBroker.FileExists(editorConfigFilePath) && !option.Force)
        {
            consoleBroker.WriteLine(".editorconfig file already exists. Please select the force option to overwrite.");
            return;
        }

        if (!option.DryRun)
        {
            await using var fs = new FileStream(editorConfigFilePath, FileMode.Create, FileAccess.Write,
                FileShare.None);
            await using var writer = new StreamWriter(fs, new UTF8Encoding(false));
            await writer.WriteAsync(editorconfigFileContent);
            await writer.FlushAsync();
        }

        this.consoleBroker.WriteLine($".editorconfig file generated at: {editorConfigFilePath}");
    }

    private readonly string editorconfigFileContent = """
                                                      root = true
