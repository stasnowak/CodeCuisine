...
public class Gitignore
{
    ...
    public async Task Default(DefaultOption option)
    {
        ...
        if (systemBroker.FileExists(gitignoreFilePath) && !option.Force)
        {
            consoleBroker.WriteLine(".gitignore file already exists. Please select the force option to overwrite.");
            return;
        }

        if (!option.DryRun)
        {
            await using var fs = new FileStream(gitignoreFilePath, FileMode.Create, FileAccess.Write, FileShare.None);
            await using var writer = new StreamWriter(fs, new UTF8Encoding(false));
            await writer.WriteAsync(gitignoreFileContent);
            await writer.FlushAsync();
        }

        this.consoleBroker.WriteLine($".gitignore file generated at: {gitignoreFilePath}");
    }

    private readonly string gitignoreFileContent = """
